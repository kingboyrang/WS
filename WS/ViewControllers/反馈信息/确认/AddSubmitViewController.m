...
- (void)viewDidLoad
{
    ...
    if (self.laugeEN) {
        self.navView.titel_Label.text = @"Confirmation";
    }else{
...
-(void)SubmitAction{
    ...
    if (self.fujianArray.count >0) {
        ...
        [queue setCompleteBlock:^{
            ...
            NSString *str = [NSString stringWithFormat:@"{XiangMuID:\"%@\",FanKuiID:\"%@\",RenYuanID:\"%@\",JieGuoQueRenXinXi:\"%@\",JieGuoQueRenID:\"%@\",FuJian:%@ }",self.xiangmuId,self.fankuiId,[UserInfo shareInstance].userId,self.myTextView.text,self.JieGuoIDstr,fujianStr];
            [params addObject:[NSDictionary dictionaryWithObjectsAndKeys:str,@"chuLiJieGuoQueRen", nil]];
            
            
            ServiceArgs *args=[[ServiceArgs alloc] init];
            args.methodName=@"ChuLiJieGuoQueRenXinXiTianJia";//要调用的webservice方法
            ...
            [manager setFinishBlock:^{
                ...
                XmlParseHelper *_helper = [[XmlParseHelper alloc] initWithData:xml];
                XmlNode *node=[_helper soapXmlSelectSingleNode:@"//ChuLiJieGuoQueRenXinXiTianJiaResult"];
...
-(void)BtnAction{
    ...
    NSMutableArray *params=[NSMutableArray array];
    [params addObject:[NSDictionary dictionaryWithObjectsAndKeys:@"5",@"fenLei", nil]];
    ServiceArgs *args=[[ServiceArgs alloc] init] ;
