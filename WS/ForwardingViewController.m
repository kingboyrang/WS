...
-(void)sendMessage{
    NSString *str = @"{XiangMuID:\"5\",RenYuanID:\"8\"}";
...
- (void)viewDidLoad
{
    ...
    self.navView.titel_Label.text = @"转发";
    UIButton *btn = [[UIButton alloc]initWithFrame:CGRectMake(250, 30, 50, 20)];
    btn.backgroundColor = [UIColor redColor];
    [btn setTitle:@"完成" forState:0];
    [self.navView addSubview:btn];
    [btn addTarget:self action:@selector(sendforwardMessage) forControlEvents:UIControlEventTouchUpInside];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([cell.isSele isEqualToString:@"1"]) {
        [cell.btn setImage:[UIImage imageNamed:@"2"]forState:0];
    }else{
         [cell.btn setImage:[UIImage imageNamed:@"1.jpg"]forState:0 ];
    }
